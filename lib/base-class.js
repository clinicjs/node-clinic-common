...
class Tool extends events.EventEmitter {
  constructor ({
    ...
    if (typeof name !== 'string') {
      throw new TypeError(`The "name" option must be of type string. Received ${typeof name}`)
    }
    if (typeof customEnvHook !== 'function') {
      throw new TypeError(`The "customEnvHook" option must be of type function. Received ${typeof customEnvHook}`)
    }
    super()
    this.name = name
